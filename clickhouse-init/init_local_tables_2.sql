...
CREATE TABLE IF NOT EXISTS testcluster_shard_1.graphite (
  ...
  Date Date,
  Timestamp UInt32
) ENGINE = ReplicatedGraphiteMergeTree('/clickhouse/tables/testcluster_shard_1/graphite', 'replica_2', 'graphite_rollup')
PARTITION BY toMonday(Date)
ORDER BY (Path, Time);
...
CREATE TABLE IF NOT EXISTS testcluster_shard_1.graphite_tree (
  ...
  Deleted UInt8,
  Version UInt32
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/testcluster_shard_1/graphite_tree', 'replica_2', 'graphite_rollup')
ORDER BY (Level, Path);
