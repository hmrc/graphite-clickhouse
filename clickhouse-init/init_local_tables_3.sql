CREATE TABLE IF NOT EXISTS testcluster_shard_3.graphite (
  ...
  Date Date,
  Timestamp UInt32
) ENGINE = ReplicatedGraphiteMergeTree('/clickhouse/tables/testcluster_shard_3/graphite', 'replica_1', 'graphite_rollup')
...
CREATE TABLE IF NOT EXISTS testcluster_shard_3.graphite_tree (
  ...
  Deleted UInt8,
  Version UInt32
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/testcluster_shard_3/graphite_tree', 'replica_1', 'graphite_rollup')
ORDER BY (Level, Path);
